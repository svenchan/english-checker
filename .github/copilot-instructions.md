...
- Data flow:
  ...
  3) API route validates classCode, builds a strict prompt (`src/config/prompts.js`), calls Groq, strips code fences, `JSON.parse`s, and `validateAndFixResponse`
...
- UI (client components):
  ...
  - `src/features/auth/components/LoginForm.js`: class code entry with inline error/loading
- Hooks:
  - `src/features/auth/hooks/useAuth.js`: stores class code in `sessionStorage`; can “validate” code by probing `/api/check`; exposes `{ classCode, isAuthenticated, login, logout, error, isLoading }`
  - `src/features/writing-checker/hooks/useChecker.js`: manages `studentText`, `isChecking`, `feedback`, and `checkWriting()` POST
- Server/API:
  - `src/app/api/check/route.js`: request queueing, class code validation, Groq call, response sanitization, Supabase logging
  ...
  - `src/config/classCodeMap.js`: maps class codes (e.g., `CLASS11`, `TEACHER`) to env var API keys; also exports `isValidClassCode`
...
- Class-scoped API keys via env variables in `src/config/classCodeMap.js` (e.g., `GROQ_API_KEY_11`, `GROQ_API_KEY_TEACHER`)
